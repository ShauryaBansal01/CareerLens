...
app.use(cors({
  ...
  credentials: true,
  // Cache-Control is not a CORS-safelisted request header, so it must be listed
  // here or the preflight fails and "Re-analyze" breaks in the browser while
  // still working from curl.
  allowedHeaders: ['Content-Type', 'Authorization', 'x-ai-provider', 'Cache-Control'],
}));
